...
async def update_stats(bot_id: str, direction: str):
    ...
    stats["history"] = history[-30:] # Последние 30 дней
...
async def bot_worker_task(bot_id: str, token: str):
    ...
    async def main_handler(m: Message):
        ...
        # 3. Пересылка админу (Livegram mode)
        if admin_id:
            try:
...
async def lifespan(app: FastAPI):
    # Восстановление активных ботов при рестарте
...
async def save_bot_api(data: dict):
    ...
    # Если бот запущен, обновляем его конфиг в памяти
