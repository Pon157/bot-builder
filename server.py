...
class BotProcessManager:
    ...
    def get_logs(self, bot_id: str, lines: int = 100):
...
async def lifespan(app: FastAPI):
    ...
    # Ждем немного чтобы Supabase URL/Key успели загрузиться если они из .env
...
async def restore_active_bots():
    ...
    try:
        ...
        if res.status_code == 200:
            for b in res.json():
                # Проверка лицензии (упрощенно)
                expires = b.get("license_expires_at") or b.get("config", {}).get("license_expires_at", 0)
...
@app.get("/api/ping")
async def ping():
    return {"status": "online", "active": len(pm.processes)}
...
async def save_bot(bot: dict):
    ...
    if not owner_id: raise HTTPException(400, "Missing owner_id")
    ...
    if res.status_code >= 400:
        ...
        raise HTTPException(500, "Database sync failed")
    return {"status": "ok"}
...
async def start_bot_ep(req: dict):
    ...
    result = await pm.start_bot(bot_id, config)
    if result is True:
        await db.patch("bots", params={"id": f"eq.{bot_id}"}, json={"status": "RUNNING"})
        return {"status": "ok"}
    else:
        raise HTTPException(500, f"Start failed: {result}")
    ...
    for bid in bot_ids:
        ...
        async with httpx.AsyncClient() as client:
            for uid in subs:
                ...
                await asyncio.sleep(0.05) # Лимит 30 мсг/сек
