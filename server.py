...
class BotManager:
    ...
    async def start_bot(self, bid: str, config: dict):
        ...
        # Бесплатные боты используют free_bot_core.py (TG) или free_vk_bot_core.py (VK)
...
async def get_user_bots(user_id: str):
    try:
        ...
        bots = res.json()
        # Исключаем VK free-plan ботов из основной TG-панели
        # (они отображаются через /api/free/vk/bots/{user_id})
        bots = [
            b for b in bots
            if not (b.get("platform") == "vk" and b.get("is_free_plan") is True)
        ]
