...
# ── YooMoney webhook для рекламных агентов ───────────────────────────────────
@router.post("/api/ads/payments/webhook")
async def ads_payment_webhook(request: Request):
    """
    YooMoney Notification (form-encoded).
    Ожидаем поля: notification_type, label (=agent_id), amount, sha1_hash
    """
    form = await request.form()
    data = dict(form)
    ...
    # Верификация SHA1
    ym_secret = os.getenv("YOOMONEY_SECRET", "")
    params_for_hash = "&".join([
    ...
    expected_hash = hashlib.sha1(params_for_hash.encode()).hexdigest()
    if expected_hash != data.get("sha1_hash",""):
        _logger().warning(f"⚠️ YooMoney: неверный SHA1 хеш")
        raise HTTPException(400, "Invalid signature")

    agent_id  = data.get("label", "")
...
@router.get("/api/ads/payments/yoomoney-url")
async def ads_yoomoney_url(authorization: str = Header(...), amount: float = 100.0):
    """Генерируем ссылку для пополнения через ЮМани."""
    agent = await _auth_agent(authorization)
    wallet = os.getenv("YOOMONEY_WALLET", "")
    if not wallet:
        raise HTTPException(503, "YOOMONEY_WALLET не настроен")
    ...
    params = {
        ...
        "sum":         str(amount),
        "label":       agent["id"],
        "comment":     "Пополнение рекламного баланса BotEngine"
    }
...
async def get_active_ad(bot_id: str = ""):
    """
    Вызывается из bot_core при каждом /start свободного бота.
    Возвращает один случайный активный пост и засчитывает показ.
    """
    db   = _db()
    ...
    # Фиксируем показ асинхронно (fire-and-forget через RPC)
    try:
