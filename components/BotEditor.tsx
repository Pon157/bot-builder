...
import { Users, UserMinus, Ban, UserCheck, Activity, AlertTriangle, Loader2, TrendingUp, ShieldCheck, Undo2, Search, Filter, Fingerprint, Database, Clock, CheckCircle2 } from 'lucide-react';
            ...
            const updatedUsers = connectedUsers.map(u => {
                if (u.id === userId) {
                    return { 
                        ...
                        warns: action === 'warn' ? (u.warns + 1) : action === 'unwarn' ? Math.max(0, u.warns - 1) : u.warns
                    };
                }
                return u;
            });
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex items-center justify-between bg-blue-600/5 border border-blue-600/10 p-4 rounded-3xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-blue-500 tracking-widest">Облачная синхронизация</p>
            <p className="text-xs text-white font-bold">Бот опрашивает панель каждые 10 секунд</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
           <CheckCircle2 className="w-4 h-4 text-emerald-500" />
           <span className="text-[10px] text-zinc-500 font-bold uppercase">Активен</span>
        </div>
      </div>
      ...
      <div className="bg-[#111] border border-zinc-800 rounded-[2.5rem] overflow-hidden">
        <div className="p-6 border-b border-zinc-800 space-y-4 bg-zinc-900/20">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 text-[9px] text-zinc-500 font-black uppercase">
