...
const BotEditor: React.FC<BotEditorProps> = ({ bot, onUpdate, onDelete, isAdminMode }) => {
  // --- [ ЛОКАЛЬНЫЙ СТЕЙТ ] ---
  // Используем локальную копию бота, чтобы инпуты не теряли фокус при вводе
  const [localBot, setLocalBot] = useState<BotConfig>(bot);
  ...
  // Синхронизация: если родитель меняет бота (например, выбрали другого в меню),
  // обновляем локальный стейт.
  useEffect(() => {
    setLocalBot(bot);
    setHasUnsavedChanges(false);
  }, [bot.id]); // Зависимость только от ID, чтобы не циклить обновления
  ...
  const safeSettings = { ...defaultSettings, ...(localBot.settings || {}) };

  useEffect(() => {
    if (activeTab === 'chat') {
        api.getBotMessages(localBot.id).then(setMessages).catch(() => setMessages([]));
    }
  }, [activeTab, localBot.id]);

  const handleToggleServer = async () => {
    setIsProcessing(true);
    try {
      if (localBot.status === BotStatus.RUNNING) {
        await api.stopBotOnServer(localBot.id);
        const stoppedBot = { ...localBot, status: BotStatus.IDLE };
        setLocalBot(stoppedBot);
        onUpdate(stoppedBot); // Обновляем родителя сразу, т.к. статус изменился
      } else {
        // При запуске всегда сначала сохраняем конфиг в БД
        const updated = await api.saveBot(localBot.owner_id, localBot);
        if (updated) {
            setLocalBot(updated);
            // НЕ вызываем onUpdate здесь, чтобы не триггерить перерисовку до старта
        }
        
        const res = await api.startBotOnServer(localBot);
        if (res === true) {
            const runningBot = { ...localBot, status: BotStatus.RUNNING };
            setLocalBot(runningBot);
            onUpdate(runningBot); // Теперь обновляем родителя
            setHasUnsavedChanges(false);
        } else {
  ...
  const syncState = async () => {
    ...
    try {
        ...
        if (updated) {
            setLocalBot(updated);
            onUpdate(updated); // Обновляем родителя только по явному сохранению
            setHasUnsavedChanges(false);
  ...
  // Эта функция теперь обновляет только ЛОКАЛЬНЫЙ стейт
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <header className="bg-[#111] border border-zinc-800 p-8 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl relative overflow-hidden">
        <div className="flex items-center gap-6 relative z-10">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center border-2 ${localBot.status === BotStatus.RUNNING ? 'bg-blue-500/10 border-blue-500/30 text-blue-500' : 'bg-zinc-900 border-zinc-800 text-zinc-600'}`}>
            <Cpu className="w-8 h-8" />
          </div>
          <div>
            <div className="flex items-center gap-3">
                <h1 className="text-3xl font-black text-white">{localBot.name}</h1>
                {isAdminMode && (
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className={`w-2 h-2 rounded-full ${localBot.status === BotStatus.RUNNING ? 'bg-blue-500 animate-pulse' : 'bg-zinc-600'}`}></span>
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">{localBot.status}</span>
            </div>
          </div>
        </div>
        <div className="flex gap-4 relative z-10">
           ...
           <button onClick={handleToggleServer} disabled={isProcessing} className={`px-10 py-4 rounded-2xl font-black text-xs uppercase transition-all flex items-center gap-2 shadow-xl ${localBot.status === BotStatus.RUNNING ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'bg-blue-600 text-white'}`}>
             <Power className="w-4 h-4" /> {localBot.status === BotStatus.RUNNING ? 'Остановить' : 'Запустить'}
           </button>
        </div>
      </header>
      ...
      <div className="min-h-[400px]">
              ...
              <section>
                ...
                <div className="space-y-5">
                  <label className="block">
                    ...
                    <input type="password" placeholder="Токен от @BotFather" className="w-full mt-2 bg-black border border-zinc-800 p-5 rounded-2xl text-white font-mono outline-none focus:border-blue-500 transition-all" value={localBot.token} onChange={e => handleLocalUpdate({...localBot, token: e.target.value})} />
                  </label>
                  <label className="block">
                    <span className="text-[10px] font-bold text-zinc-500 uppercase ml-2">ID Группы Админов (Forum)</span>
                    <input type="text" placeholder="-100..." className="w-full mt-2 bg-black border border-zinc-800 p-5 rounded-2xl text-white outline-none focus:border-blue-500 transition-all" value={localBot.adminChatId} onChange={e => handleLocalUpdate({...localBot, adminChatId: e.target.value})} />
                  </label>
                  <label className="block">
                    <span className="text-[10px] font-bold text-zinc-500 uppercase ml-2">Приветствие (/start)</span>
                    <textarea className="w-full mt-2 bg-black border border-zinc-800 p-5 rounded-2xl text-white min-h-[100px] outline-none text-xs focus:border-blue-500 transition-all resize-none" value={localBot.welcomeMessage || ""} onChange={e => handleLocalUpdate({...localBot, welcomeMessage: e.target.value})} />
                  </label>
                </div>
              </section>
        ...
        {activeTab === 'interface' && (
          <div className="space-y-6">
             <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => handleLocalUpdate({...localBot, buttons: [...(localBot.buttons || []), {text: '', response: '', type: 'message'}]})} className="bg-blue-600 px-8 py-4 rounded-2xl text-[11px] font-black text-white uppercase flex items-center gap-2 shadow-lg shadow-blue-600/20">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {(localBot.buttons || []).map((btn, i) => (
                 <div key={i} className="bg-[#0d0d0d] border border-zinc-800 rounded-[2.5rem] p-8 space-y-6 relative group border-t-4 border-t-blue-500/20 shadow-xl">
                    <button onClick={() => handleLocalUpdate({...localBot, buttons: localBot.buttons.filter((_, idx) => idx !== i)})} className="absolute top-6 right-6 text-zinc-600 hover:text-rose-500 transition-colors"><X className="w-5 h-5" /></button>
                    <div className="space-y-5">
                        <label className="block"><span className="text-[9px] font-bold text-zinc-600 uppercase ml-2">Текст на кнопке</span><input className="w-full mt-2 bg-black border border-zinc-800 p-5 rounded-2xl text-white text-sm font-bold outline-none focus:border-blue-500" value={btn.text} onChange={e => { const nb = [...localBot.buttons]; nb[i].text = e.target.value; handleLocalUpdate({...localBot, buttons: nb}); }} /></label>
                        <label className="block"><span className="text-[9px] font-bold text-zinc-600 uppercase ml-2">Ответ системы</span><textarea className="w-full mt-2 bg-black border border-zinc-800 p-5 rounded-2xl text-white text-sm min-h-[120px] outline-none focus:border-blue-500 resize-none" value={btn.response} onChange={e => { const nb = [...localBot.buttons]; nb[i].response = e.target.value; handleLocalUpdate({...localBot, buttons: nb}); }} /></label>
                        <div className="flex bg-black p-1 rounded-xl border border-zinc-800">
                          {['message', 'request'].map(type => (
                            <button key={type} onClick={() => { const nb = [...localBot.buttons]; nb[i].type = type as any; handleLocalUpdate({...localBot, buttons: nb}); }} className={`flex-1 py-2.5 rounded-lg text-[9px] font-black uppercase transition-all ${btn.type === type ? 'bg-blue-600 text-white' : 'text-zinc-600'}`}>{type === 'message' ? 'Обычный ответ' : '🆘 Заявка (Тикет)'}</button>
                          ))}
                        </div>
                    </div>
                 </div>
        ...
        {activeTab === 'logic' && (
          <div className="space-y-6">
            <div className="flex justify-between items-end mb-6"><h2 className="text-2xl font-black text-white uppercase">Триггеры авто-ответа</h2><button onClick={() => handleLocalUpdate({...localBot, triggers: [...(localBot.triggers || []), {keyword: '', response: ''}]})} className="bg-emerald-600 px-8 py-4 rounded-2xl text-[10px] font-black text-white uppercase flex items-center gap-2 transition-all"><Plus className="w-4 h-4" /> Новый триггер</button></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {(localBot.triggers || []).map((trig, i) => (
                 <div key={i} className="bg-[#0d0d0d] border border-zinc-800 rounded-[2.5rem] p-8 space-y-5 relative border-t-4 border-t-emerald-500/20 shadow-xl">
                    <button onClick={() => handleLocalUpdate({...localBot, triggers: localBot.triggers.filter((_, idx) => idx !== i)})} className="absolute top-6 right-6 text-zinc-600 hover:text-rose-500"><X className="w-5 h-5" /></button>
                    <input placeholder="Ключевое слово (например: цена)" className="w-full bg-black border border-zinc-800 p-5 rounded-2xl text-white text-sm font-bold outline-none focus:border-emerald-500" value={trig.keyword} onChange={e => { const nt = [...localBot.triggers]; nt[i].keyword = e.target.value; handleLocalUpdate({...localBot, triggers: nt}); }} />
                    <textarea placeholder="Что бот должен ответить..." className="w-full bg-black border border-zinc-800 p-5 rounded-2xl text-white text-sm outline-none min-h-[120px] focus:border-emerald-500" value={trig.response} onChange={e => { const nt = [...localBot.triggers]; nt[i].response = e.target.value; handleLocalUpdate({...localBot, triggers: nt}); }} />
                 </div>
               ))}
            </div>
          </div>
        ...
        {activeTab === 'stats' && <BotStatsView bot={localBot} onUpdate={onUpdate} />}
