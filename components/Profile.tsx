...
const Profile: React.FC<ProfileProps> = ({ user, bots, onUpdateBots }) => {
  ...
  // ref чтобы polling остановился при размонтировании
  const pollingActive = useRef(false);

  const fetchWallet = useCallback(async (): Promise<number | null> => {
    try {
  ...
  // Запускаем один раз при маунте — пустой dep array намеренно
  useEffect(() => {
    pollingActive.current = true;

    const init = async () => {
      // Один запрос на balance + прайс параллельно
      const [walletData, pricesData] = await Promise.all([
        fetch(`/api/payments/balance/${user.id}`).then(r => r.json()).catch(() => null),
        fetch('/api/payments/prices').then(r => r.json()).catch(() => []),
      ]);

      if (pricesData?.length) setPrices(pricesData);
  ...
  const handleBuy = async (serviceKey: string) => {
    if (!buyBotId) { alert('Выберите бота'); return; }
  ...
  return (
    <div className="space-y-8 md:space-y-12 animate-in fade-in duration-500 pb-10">
      ...
      {/* Баннер polling */}
      {isPolling && (
        <div className="flex items-center gap-3 p-4 bg-yellow-950/30 border border-yellow-700/40 rounded-2xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          ...
          {activeSection === 'wallet' && (<>
            <section className="bg-[#121212] border border-zinc-800 rounded-[2rem] p-6 md:p-8 shadow-2xl space-y-5">
              ...
              <div className="flex gap-3">
                <input type="number" min={10} max={100000} placeholder="Другая сумма, ₽"
                ...
                <button onClick={handleTopup}
                  ...
                  {isInitiating ? <RefreshCw className="w-4 h-4 animate-spin" /> : <><Plus className="w-4 h-4" />Оплатить</>}
                </button>
              </div>
            ...
            <section className="bg-[#121212] border border-zinc-800 rounded-[2rem] p-6 md:p-8 shadow-2xl">
              <h3 className="text-lg font-bold text-white mb-5 flex items-center gap-2">
                <Clock className="w-5 h-5 text-zinc-500" />История транзакций
              </h3>
              {transactions.length === 0 ? (
                ...
                <div className="space-y-2">
                  {transactions.map(tx => {
                    ...
                    return (
                      <div key={tx.id} className={`flex items-center justify-between p-4 border rounded-2xl transition-all ${
                        isPendingTopup ? 'bg-yellow-950/20 border-yellow-800/30'
                        : isFailed     ? 'bg-red-950/10 border-red-900/20'
                        :                'bg-black border-zinc-800'
                      }`}>
                        <div className="flex items-center gap-3">
                          {isPendingTopup ? <Clock className="w-4 h-4 text-yellow-500 flex-shrink-0 animate-pulse" />
                           : isFailed      ? <XCircle className="w-4 h-4 text-red-500 flex-shrink-0" />
                           : tx.type === 'topup' ? <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0" />
                        ...
                        <div className="text-right">
                          ...
                          {isCompleted && <p className="text-[10px] text-zinc-600">→ {tx.balance_after.toFixed(2)} ₽</p>}
                        </div>
                      </div>
                    );
                  })}
          ...
          {activeSection === 'license' && (<>
            <section className="bg-[#121212] border border-zinc-800 rounded-[2rem] p-6 md:p-8 shadow-2xl space-y-5">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <BotIcon className="w-5 h-5 text-blue-500" />Ваши боты
              </h3>
              <div className="space-y-2">
                {bots.map(bot => {
                ...
                {bots.length === 0 && <p className="text-sm text-zinc-600 text-center py-4">Ботов нет</p>}
              </div>
              <div className="border-t border-zinc-800 pt-5 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {pricesByGroup.bot.map(p => (
                    <button key={p.service_key} onClick={() => handleBuy(p.service_key)}
          ...
          {activeSection === 'ai' && (<>
            <section className="bg-[#121212] border border-zinc-800 rounded-[2rem] p-6 md:p-8 shadow-2xl space-y-5">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Brain className="w-5 h-5 text-purple-500" />AI-токены
              </h3>
              <div className="space-y-2">
                {bots.map(bot => (
                  <div key={bot.id} className="flex items-center justify-between p-4 bg-black border border-zinc-800 rounded-2xl">
          ...
          {activeSection === 'miniapps' && (<>
            <section className="bg-[#121212] border border-zinc-800 rounded-[2rem] p-6 md:p-8 shadow-2xl space-y-5">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <AppWindow className="w-5 h-5 text-indigo-400" />Мини-приложения
              </h3>
              <div className="space-y-2">
                {bots.map(bot => {
                  ...
                  return (
                    <div key={bot.id} className="flex items-center justify-between p-4 bg-black border border-zinc-800 rounded-2xl">
                      <span className="text-sm text-white">{bot.name}</span>
                      <span className={`text-xs font-bold ${lic?.active ? 'text-green-400' : 'text-red-400'}`}>
                        {lic?.active && lic?.expires_at ? `до ${new Date(lic.expires_at).toLocaleDateString('ru-RU')}` : 'Неактивно'}
                      </span>
                    </div>
                  );
                })}
