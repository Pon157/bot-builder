  ...
  if (error || !appData) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0a0f', flexDirection: 'column', gap: 16 }}>
        <p style={{ color: '#ef4444', fontSize: 18, fontWeight: 700 }}>{error}</p>
      </div>
    );
  }
        ...
        <div style={{
          ...
          {submitted ? (
            <div style={{
              ...
              <p style={{ color: theme.textPrimary, fontSize: 22, fontWeight: 800, margin: '0 0 8px' }}>Отправлено!</p>
              <p style={{ color: theme.textSecondary, fontSize: 15, margin: 0 }}>Ваши данные успешно получены.</p>
              <button
                onClick={() => { setSubmitted(false); setFormData({}); }}
                style={{ marginTop: 24, background: theme.primary, color: '#fff', border: 'none', borderRadius: theme.radius, padding: '12px 28px', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}
