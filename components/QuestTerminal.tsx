...
const QuestTerminal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Инструкция: "binary_to_caesar_minus_3"
    ...
    // "repair_module" (в Цезаре: uhsdlu_prgxoh)
    ...
    (window as any).repair_module = () => {
      ...
      if (k === "Hjdvdosi3245") {
        ...
        // "Hjdvdosi3245" (в Цезаре: Kmgygrvl6578)
        ...
        Object.defineProperty(window, k, {
          get: () => {
            // "execute_recovery" (в Цезаре: hahfxwh_uhfryhub)
            ...
            (window as any).execute_recovery = () => {
                // "xbsiOn)ytvjEw#bihDu" (в Цезаре: aevlRq)bwymHz#e~lkGx)
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#0f0', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace', overflow: 'hidden', position: 'relative' }}>
      ...
      {noise.map((p, i) => (
        <div key={i} className="bg-noise" style={{ 
          position: 'absolute', 
          bottom: '-60px', 
          left: p.x, 
          color: '#002200', 
      ...
      <div style={{ border: '1px solid #0f0', padding: '50px', background: '#050505', textAlign: 'center', width: '450px', zIndex: 1, boxShadow: '0 0 50px rgba(0,255,0,0.1)' }}>
        <h1 style={{ marginBottom: '30px', letterSpacing: '8px', fontSize: '18px' }}>ТЕРМИНАЛ_ЯДРА_V12</h1>
