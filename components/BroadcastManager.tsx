...
const BroadcastManager: React.FC<BroadcastManagerProps> = ({ bots }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <header>
        <h1 className="text-3xl font-black text-white">Глобальная рассылка</h1>
        <p className="text-zinc-500 text-sm">Отправка сообщений всем пользователям выбранных инстансов.</p>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 bg-[#111] border border-zinc-800 p-8 rounded-[2rem] space-y-6">
          <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block">Текст сообщения</label>
          <textarea 
            className="w-full bg-black border border-zinc-800 rounded-2xl p-6 text-sm focus:ring-1 focus:ring-blue-500 min-h-[250px] resize-none"
          ...
          <button 
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#111] border border-zinc-800 rounded-[2rem] p-4 space-y-2">
            {bots.filter(b => b.status === 'RUNNING').map(bot => (
              ...
              >
                <span className="text-xs font-bold text-zinc-300">{bot.name}</span>
                <input type="checkbox" className="hidden" checked={selectedBotIds.includes(bot.id)} onChange={() => toggleBot(bot.id)} />
                <div className={`w-5 h-5 rounded-lg border flex items-center justify-center ${selectedBotIds.includes(bot.id) ? 'bg-blue-600 border-blue-600' : 'border-zinc-800'}`}>
